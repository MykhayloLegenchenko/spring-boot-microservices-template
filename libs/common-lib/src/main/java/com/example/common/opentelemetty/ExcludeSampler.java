package com.example.common.opentelemetty;
...
class ExcludeSampler implements Sampler {
  ...
  ExcludeSampler(ConfigProperties config) {
    ...
    serverPaths = getPatterns(config, "server.paths");
    clientHosts = getPatterns(config, "client.hosts");
    clientPaths = getPatterns(config, "client.paths");
  }
  ...
  private static List<Pattern> getPatterns(ConfigProperties config, String propertyName) {
    var property = config.getString("otel.exclude-sampler." + propertyName);
    if (property == null) {
  ...
  private static class RootSampler implements Sampler {
    @Override
    public SamplingResult shouldSample(
        Context parentContext,
        String traceId,
