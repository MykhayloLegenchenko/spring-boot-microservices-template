...
class ClientInterfaceProcessor {
  ...
  private void process() {
    ...
    if (Objects.requireNonNull(rootElement.getEnclosingElement()).getKind()
        != ElementKind.PACKAGE) {
  ...
  private JCTree processElement(Element el) {
    return el.accept(
        new SimpleElementVisitor9<JCTree, @Nullable Void>() {
          @Override
  ...
  private static List<? extends AnnotationMirror> getAnnotationMirrors(
      @Nullable Element el, Collection<String> annotationTypes) {
